...
begin
  ...
  if not exists (
    ...
    insert into task_slides (
      ...
      (seeded_task_id, 2, $copy$MVP Studio — where your Pilates journey begins. / Beginner · Intermediate · Advanced reformer & mat classes, for every stage of your practice.$copy$, $copy$Reformer or mat class photo, ideally showing different class levels side by side$copy$, $copy$MVP Studio: reformer & mat, every level welcome.$copy$, null, null),
      (seeded_task_id, 3, $copy$MVP Training — for those going further. / Polestar Pilates certification. Gyrotonic training. Programs for clients ready to deepen their practice — or become instructors themselves.$copy$, $copy$Training/teaching photo showing the range of programs$copy$, $copy$MVP Training: go deeper, or become an instructor.$copy$, null, null),
      (seeded_task_id, 4, $copy$Same instructors. Same community. Just clearer about how we show up for you.$copy$, $copy$Warm studio environment photo$copy$, null, null, null),
      (seeded_task_id, 5, $copy$Come see it for yourself. Link in bio.$copy$, $copy$Studio photo, welcoming tone$copy$, $copy$See you soon 🤍$copy$, null, null);
  end if;
  ...
  if not exists (
    ...
    insert into tasks (
      ...
      $copy$Explain what the Polestar certification means and build credibility, ending with a CTA for the upcoming Teacher Training Program.$copy$,
      'not_started',
      $copy$Ever wondered what "Polestar Pilates" means on our schedule? 🧠 Polestar is one of the most respected Pilates certifications in the world — rooted in rehabilitation science, not trends. It means our instructors are trained to understand how your body actually moves, and to teach accordingly. And if you've ever thought about going further — our Polestar Teacher Training Program is opening enrollment soon. Link in bio for details. #PolestarPilates #MotionVitalityPilates #TeacherTraining$copy$,
    ...
    insert into task_slides (
      ...
      (seeded_task_id, 3, $copy$Every MVP instructor is Polestar-certified, trained to teach with precision and adapt to every body.$copy$, $copy$Instructor teaching photo$copy$, null, $copy$Confirm exact wording with Gary/Dorothy before publishing$copy$, null),
